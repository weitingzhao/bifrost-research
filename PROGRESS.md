...
**Status:** 🟢 Wave D complete and reviewed — D1–D4, the post-program fix batch, and the Owner's three-part cost-and-readability program released as **0.78.0** (api · mcp · engines; dagster `0.75.0-dagster`), frontend D1–D4 in `bifrost-trade-frontend` (`36c0de0`, Satellite chain). Open: D3 acceptance waits on the Owner's L0 (trust matrix still L1) and one unattended day; the DeepSeek $0.50/day cap makes judge fallback (= dissent) the norm after the first run. Program next: Owner sign-off on D3 (Ops Console → Ops Desk → **Trust & Autonomy** → the *Research · Loop Batch* row → set **L0** in the Set-level column), then a real unattended day. The write is `PUT /api/v1/agent/governance/trust-overrides/research-loop-batch` and an operator token is enough; an agent still does not click it, because the same token can scale the trade daemon and grant the Loop the autonomy the gate exists to withhold.

**Owner-authorised housekeeping, 2026-09-07.** `research.agent_persona` moved from `postgres` to `bifrost`, so all 15 tables in the schema share one owner. The DDL then ran to completion for the first time: `candidate_pool`, `copilot_bridge_event` and the three `playbook_*` tables now exist. They had been missing because `CREATE EXTENSION vector` needs superuser, its exception was swallowed, and PostgreSQL had already aborted the transaction — every later statement died and the Job still reported success. Savepoints now bound that block and both best-effort GRANT loops. `research.embedding_chunk` is still absent: pgvector 0.8.2 is available in the cluster but not installed, and installing it needs the Owner. The Inbox backlog is cleared, 263 pending drafts down to 54, the 209 routine briefings from 09-01→09-04 dismissed one at a time through the API so each leaves a ledger row; everything kept is live decision material.


**Owner's usage program, 2026-09-07 (`0.78.0`).** Three changes in the order the Owner set, each measured rather than estimated.

*Triage.* A judge is an agent that calls tools in a loop, so it costs about $0.077 a candidate; the loop sent every proposed candidate to both judges and nothing asked first which were worth it. A new stage ranks them in one tool-free call for **$0.00014**. Advisory by default — `policy.triage.deep_judge_top_n` is 0, so a run behaves and bills exactly as before and only gains a ranking. With a cap set, held candidates carry no verdicts, which the leash already treats as not-accepted. Every failure path judges everything: a triage that cannot rank must not narrow.

*Bilingual judges.* The obvious fix was a second call to translate, and it was the wrong one. A run's bill is almost entirely input tokens, because each judge re-sends its conversation on every tool round; output is about two percent. Asking for both languages in the one reply moved output from ~1,694 tokens a candidate to 1,718, **up 1.4%**. `summary_zh` is absent, never empty, when a judge did not write one, so the reader falls back to English and is told.

*Price before the button.* `GET /objectives/{id}/run-estimate` answers from this objective's own run history rather than a constant, since the rate depends on how much evidence its candidates carry. Eight candidates across both judges reads **$0.49** from five runs; three with `gpt-4o-mini` alone reads **$0.07**. `POST batch-run` now takes `judge_models`, `deep_judge_top_n` and `symbols`, folded into a copy of the policy and never written back — a run shaped by hand does not become tomorrow's scheduled behaviour.

**Bifrost Rating and the decision memo, 2026-09-07 (`0.79.1`, frontend `dd7f4f1`).** Owner's option A. Every candidate now carries a grade (the SEPA grade), 1–5 stars of conviction from evidence first and judges second, an action from grade × stars × where price sits against the pivot, entry zone / stop / 2R target from the run's own price map, and an outlook from score drift within the same objective. A pure function of the run's record; no model sets a price. The drawer leads with a one-sentence verdict written from the ratings, then a ranked deck of cards with the full case behind each; the seven stages fold beneath as the audit trail. The API pod now runs the same judges as the CronJob, so a run started from the Run dialog is judged by the models it was priced for. First judged run on DEV: `run_1a07d87d970f09ffc` — three names, none above ★★, validate blocked two.

**Three levels of autonomy, 2026-09-07 (`0.80.0`, frontend `f0a8cd8`).** The Research menu is regrouped by how much the system does for the Owner — Autopilot · unattended, Copilot · on request, Workbench · by hand — with routes unchanged. Harness Console is Autopilot and sits first. The page leads with standing: the ladder, trust read from the cluster matrix and named as such, the next unattended run, today's purse against its caps, memos waiting; then one brief per objective — what it hunts (from its policy), what it said last (the last rated run's headline and picks), whether its picks have been right (`candidate_outcome`, objective-scoped, labelled when the harness-wide record stands in), what it costs. `GET /research/loop/autopilot`, all reads. Found on the way: the Owner's L0 was set on the local dev platform-api, not the cluster's — the unattended loop reads the cluster and still sees L1; and force-deleting a run cascades to its candidates' settled outcomes, which erases track record.
...
| D | Copilot in the loop, and the leash | **D1 ✅ 2026-09-07** (backend `0d68e70` + release `0.73.0`: MCP read tools `research.loop.list_runs` / `get_run` / `explain_candidate` over `copilot/harness/run_digest.py` — a run folded into one object (plan provenance, funnel, hit-rate gate, every candidate with evidence and each judge's stance per model, report why / price / settled / wrong_if, drafts; explain adds the candidate row, hypothesis, settled validation, `what_would_unmake_it`), nothing re-derived from live data; write tool `research.loop.run_objective` (dry_run preview; execution needs the Owner's approval token, refuses a batch pass; shares `batch_orchestrate.start_async_batch` with the HTTP batch-run); loop_curator explain flow, triage routes Loop questions, verdict gets `loop_specialist`. Frontend `40beb44`: per-candidate Ask button in the Inbox batch → Copilot with run + symbol snapshot and an explain prompt; drawer Discuss prompt starts with `get_run`. Acceptance on the local api against DEV data: 'why was RKLB proposed and what would unmake it' on `run_1a078ead64edd0578` → one `explain_candidate` call, answer cites evidence.selection / price_context / track_record, names the gpt-4o-mini portfolio dissent, quotes wrong_if (`path leaves AVOID`, `close breaks below the 50-day (75.24)`), states validate did not block) · **D2 ✅ 2026-09-07** (backend `60523a3` + release `0.74.0`: `copilot/agents/daily_digest.py` posts one `daily_digest` draft per trading day — holdings ∪ candidates proposed since yesterday through `build_exhibit` (iv_rank · vrp · gex_regime · terrain_regime), Loop state (objectives, runs, drafts waiting, trust), candidate batches folded by objective + symbol set, dissents one line per judge model with the report's wrong_if, hypotheses the outcome rule resolved since yesterday; facts by code, DeepSeek rewrites the prose through the planner's endpoint table (≈ $0.001, cost on the payload) and the heuristic markdown stands otherwise; second run the same day is a no-op unless forced; Morning Prep folded in — the digest holds the 11:30 UTC Dagster slot (`research_daily_digest_schedule`); `POST /research/agents/digest/run`. Frontend `856ba67`: `DailyDigestBody` (prose, then the batches folded beneath with Pipeline links), Decision Inbox opens on Briefings with the digest first when one is pending, Copilot `InboxBanner` sorts the digest first and reads 'Daily digest · N more pending'. DEV acceptance: digest `drf_1a07987057bfdfaa074` for 2026-09-07 (11 symbols, 2 batches, 11 dissents, 0 resolutions, deepseek-chat prose 333 words); second post skipped; Inbox on `:5173` opened on it with the two batches folded beneath; `research_daily_digest_schedule` RUNNING on DEV Dagster) · **D3 🟡 code on DEV 2026-09-07, acceptance pending Owner L0** (backend `a193b1b` + release `0.75.0`: `copilot/harness/leash.py` decides per candidate — judges agree ∧ validate did not block ∧ selection evidence measured ∧ source settled hit rate at its longest judged horizon (≥ 5 outcomes) ≥ `policy.min_source_hit_rate` (default 0.45, both whitelists); `approve_all_for_run` promotes the names that pass, keeps the rest on the draft with reasons (draft stays pending), no more run-level all-or-nothing, `loop_auto_accept` ledger row, `outputs.approve_all.{accepted,held_symbols}` quoted by the digest; `copilot/agents/weekly_policy_review.py` Sunday 22:00 UTC per objective on its own settled outcomes (`build_summary(objective_id=…)`) → policy_suggestion with evidence or a ledger row, never auto-approved, `POST /research/agents/weekly-policy/run`. Frontend `53c617a`: batch rows tagged accepted / held with reasons, run drawer names accepted / held, Decision Inbox opens on Decisions with a strip to the digest, `min_source_hit_rate` knob. DEV: `research_weekly_policy_review_schedule` RUNNING; first review 2026-W37 proposed nothing (stock explorer 8 judged at T+1 hit 0.50; IV watch 0 settled). Dry pass of the leash over today's batches: nothing would be accepted — every name held on `judges did not agree (dissent)` (DeepSeek's $0.50 cap exhausted → fallback = dissent) or `validate blocked` (EE) or `selection evidence not measured` (IWM, QQQ). Trust matrix: research-loop-batch still **L1** — the Owner's Console click is the remaining step; then one unattended trading day. **Verified 2026-09-07 on the unattended path** (`kubectl create job --from=cronjob/research-harness`, job `research-harness-d3-235648`): with the purse sized at $1.50 the judges actually judge — `deepseek-chat ok=7/8 fallback=1 cap_exceeded=0 $0.5552`, `gpt-4o-mini ok=8/8 $0.0760`, agreement 2 agree / 6 dissent against yesterday's 0 agree when starved. One run alone costs more than the old $0.50 cap, which is exactly why every candidate was held. The leash then accepts for the first time: a dry pass over the batch accepts **BG** on all four conditions and holds the other seven with the failed condition named (NVDA / EE dissent; WT validate blocked; PAYS / FANG / HALO / LPG both). The run still ends `Auto-approve skipped — not at Trust L0`: the platform refuses the matrix write to an agent (`role=operator, can_admin=false`) and the auto-mode classifier blocked the manifest override commit, so both doors are Owner-only by design. Both objectives completed: stock explorer 8 candidates (deepseek ok=7/8 $0.5552, gpt-4o-mini ok=8/8 $0.0760, 2 agree / 6 dissent, leash accepts BG), IV watch 3 candidates (deepseek ok=2/3 $0.2002, gpt-4o-mini ok=3/3 $0.0313, 0 agree / 3 dissent, leash accepts none). **A full unattended day costs $0.7553 DeepSeek + $0.1074 OpenAI** — the old $0.50 cap could not fund even the first objective, and $1.50 leaves about half a day of headroom. The leash accepted 1 of 11: `validate blocked` is the dominant gate (6 of 11), not judge disagreement, and 7 of 11 candidates carry no lens reading at all. That acceptance rate is the system honestly reporting that its evidence is too thin to act alone — the fix is option coverage, not a looser leash. Weekly policy review ran for 2026-W37 and proposed nothing — correct, the stock explorer's 9 settled outcomes hit 56%) · **D4 ✅ 2026-09-07** (backend `939f39c` + release `0.76.0`: `GET /research/verdicts/{symbol}` over `copilot/agents/symbol_verdicts.py` — the day's digest lines for the symbol (lens bands, proposed by the Loop, what the leash decided, judges split, resolution) and every proposal with its state (candidate proposed / accepted / dismissed, hypothesis active / validated / rejected by rule, decision / order drafts awaiting approval, the chat approval ledger previewed / approved / executed / failed), read from the Inbox's tables. Frontend `36c0de0`: `CopilotVerdictStrip` under every LabHub's context bar, one chip per statement with tone and a link to where it is decided, refetched every minute. Acceptance on `:5173` against DEV: a `research.loop.propose_candidate` approved and executed through `/research/copilot/approve` + `/execute` for AAPL shows on the AAPL vol-regime view as 'Candidate · proposed (Copilot)' + 'propose candidate · executed' within one refresh. Two chat-write defects found on that path and fixed in `0.76.1`: `_dispatch_tool` read mcp 1.29's `(content, structured)` tuple as `[]` so every chat-approved write reported success, wrote nothing and logged executed (`4526d43`); the approval hash bound the token to the arguments the Owner saw while the tool re-hashed them with defaults filled in, so any proposal omitting an optional argument failed as tampering (`278a436`)) |
