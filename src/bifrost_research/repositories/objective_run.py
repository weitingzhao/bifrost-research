"""SQL layer for ``research.objective_run`` — one run of an objective.

Split from ``objective.py`` (0.113.0) when that file reached the 800-line
line; ``objective.py`` re-exports every name here, so ``obj_repo.get_run``
and the rest keep working for their callers.
"""
