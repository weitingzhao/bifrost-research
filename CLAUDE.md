...
- 版本：`0.97.0`（**索引解包扫尾 —— 34 处谓词** —— 0.96.0 只解开了 `repositories/opex_cycle.py` 的 6 处；剩下 43 处里 **34 处在谓词位**（`WHERE` / `AND` / `OR` 后），另 8 处是 `SELECT` 投影（规范化输出而非过滤输入，不包索引，保留），1 处是 opex_cycle 的注释。**先验后改**：19 个表/列全部实测 `col IS DISTINCT FROM UPPER(TRIM(col))` = 0 行 —— `raw_market.stock_daily` 1362.6 万、`option_daily` 352.0 万、`option_open_interest` 203.4 万、`option_snapshot` / `v_option_snapshot_with_stock` 105.4 万、`option_contract` 73.3 万、`stock_financials` 65.8 万、`option_iv_reconstructed_daily` 65.8 万 —— 且每个调用方都已 `.strip().upper()`。pod 内真实函数计时（2026-09-04 有数据日，交替 min-of-3）：`event_query._resolve_earnings_events` 222.1s → 0.598s、`event_query._fetch_stock_price` 84.7s → 0.053s、`vrp.fetch_stock_daily_closes` 54.2s → 0.098s、`momentum.fetch_daily_bars` 49.3s → 0.193s、`iv_solver.project_vendor_snapshot_window` 14.2s → 3.80s、`atm_iv.fetch_snapshot_iv_rows` 9.10s → 1.50s、`atm_iv.fetch_reconstructed_iv_rows` 7.90s → 0.398s、`gex.fetch_spot` 3.40s → 0.001s、`event_query._pick_option` 2.90s → 0.002s、`surface.fetch_iv_points_for_date` 3.05s → 0.596s。**没有一处变慢**：`flow.fetch_flow_rows`（median 10.2s → 4.06s）与 `pcr.fetch_volume_totals_for_date`（median 17.2s → 11.8s）需要 min-of-7 才看得准，单次 min 会被共享集群的抖动骗过去。新增 `tests/test_sargable_symbol_predicates.py` 作棘轮 —— 只拦谓词位，不拦投影位。**两处剩余瓶颈不在本次范围**：① `raw_market.option_open_interest` 没有 `underlying` 索引，两种写法都全扫该分区（D13 只读 `raw_market.*`，不建索引，留给 ingest 侧）；② `option_snapshot` 上的 `DATE(timezone('America/New_York', snapshot_ts)) = %s` 是同一类非 sargable 谓词，实测才是 pcr / 快照路径的主导开销（flow 的 LATERAL 一次就打 14.3 万 buffer）。D10 仍 BLOCKED）
- 版本：`0.96.0`（**Dossier 首屏 21s → 一次批量** —— 实测归因推翻了「12 个请求太多」的假设：12 个 lens 的 `build_exhibit` 共 23.0s，其中 `opex_pin` 一个占 20.6s，而全部 track_record 加起来只有 0.24s。根因在 `repositories/opex_cycle.py`：谓词写成 `UPPER(TRIM(symbol)) = %s`，把索引包死；调用方本来就 `.strip().upper()`，且 `raw_market.stock_daily`（1360 万行）与 `features.option_metric_max_pain_daily` 实测 `symbol IS DISTINCT FROM UPPER(TRIM(symbol))` 均为 0 行 —— 包装什么也没买到，只把 0.00s 的点查变成 3.4s 的扫描。该文件 6 处谓词全部解开。**批量**：`/research/exhibit/composite` 从「一条连接顺序跑完」改为轮转分片 + 4 个 worker，每 worker 一条连接（连接约 0.5s，与读一个 lens 同量级；12 个独立请求要付 12 次）；顺序按请求返回，单个 lens 失败降级为 `missing` 存根而不拖垮邻居，数据库整体不可达仍是 503 而不是 12 条借口。前端 Dossier 改为一次批量并回种各 lens 的 query key，hub 缓存不受影响；D10 仍 BLOCKED）
