...
**快照切到宇宙（2026-09-08 20:20 UTC，Plugin 0.17.1）**：六个期权面读的是当日链快照与 OI，由 `eod-pipeline` 产出，此前仍按 27 个自选股跑，回填再多也变不成分析面。现在 `eod-pipeline` 以 `universe: research` 快照 575 个名字：常驻整条链，核与边用**近价窗口**（下 3 个到期、行权价在最新收盘 ±15%），窗口在 vendor 端生效（`strike_price.gte/lte`、`expiration_date.lte`），随 payload 跨分页续传。整链 575 个约 130 GB 的 90 日留存，窗口约 18 GB。窗口查询经过三次修正才在 pod 里稳定：0.17.1 分块 40 个一条仍每块 41 秒（合约表当日增长二十倍、统计信息过期，`ANY(40)` 不走索引）；0.17.2 改为逐标的按 `(underlying, expiry)` 索引点查；0.17.3 把 spot 查询限定在最近 7 天（无界的 `DISTINCT ON` 扫 1,360 万行超时）。最终 548 个名字 32.6 秒，538 个有窗口，10 个整链（9 个无期权 + CRNX 到期不足 3 个）。option worker 4 → 8（manifest 同步）。今晚 22:00 UTC 首次按宇宙快照。

**回填已排入（2026-09-08 19:40 UTC，Plugin 0.16.2）**：`option-backfill` 一次排入 13,548 个计划任务——常驻 27 × 24 个月（优先级 4）、核 527 × 24（优先级 3）、边 21 × 12（优先级 2）。0.16.2 修的是第三个当天缺陷：1.4 万行的单条 INSERT 撞 2 秒角色超时，改为每 2,000 行一条、事务内 60 秒。计划任务展开出的聚合任务上限优先级 2，排在今晚指数回填（优先级 1，约 60 万）之前、当日 EOD 链之后。
