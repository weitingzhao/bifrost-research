...
* **Momentum** (M) — reads ``features_signals.momentum_score_daily`` composite score
  (Bifrost Momentum Radar; falls back to neutral 50 when missing).
* **Options Structure** (O) — combines IV percentile, PCR, and GEX wall
  proximity from ``features_daily.*`` / ``features_option.gex_levels_daily``.
...
D10 BLOCKED — advisory writes to ``features_signals.sepa_score_daily`` only.
"""
